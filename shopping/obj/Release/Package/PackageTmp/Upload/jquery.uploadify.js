...
*/
function formatBytes(bytes) {      //将字节数据格式化成带单位的大小数据
//    var s = ['Byte', 'KB', 'MB', 'GB', 'TB', 'PB'];
//    var e = Math.floor(Math.log(bytes) / Math.log(1000));
//    return (bytes / Math.pow(1000, Math.floor(e))).toFixed(3) + " " + s[e];
...
(function ($) {
    ...
    var handlers = {
        ...
        onSelect: function (file) {
            ...
            // Get the size of the file    获取的大小不够精准，自己重新用formatBytes来获取
            ...
            fileSize += suffix;
            fileSize=formatBytes(file.size); //用自定义的函数重新格式化文件大小，
        ...
        onUploadProgress: function (file, fileBytesLoaded, fileTotalBytes) {
            ...
            if ($.inArray('onUploadProgress', settings.overrideEvents) < 0) {
                ...
                if (fileBytesLoaded == fileTotalBytes)
                    saveMsg = "正在保存数据<img src='saving.gif' height='15px'/>"
                //                if(fileBytesLoaded<=100*1024)
                //                        saveMsg="准备上传……"
                strUpload=formatBytes(fileBytesLoaded);
                if (settings.progressData == 'percentage') {
                    $('#' + file.id).find('.data').html(' - '+'('+strUpload+')-' + percentage + '%' + saveMsg);
